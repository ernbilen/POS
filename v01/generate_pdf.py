...
def generate_invoice(invoice_data,PO_number, extra_lines,path):
    ...
    add = addresses.copy() # copy the stored address
    ...
    can.drawRightString(middleCol, secondLine, "{}%".format(vat))
    can.drawRightString(rightMargin, secondLine, "${:.2f}".format(float(vat)))
    ...
    can.drawRightString(rightMargin, totalLine, "${:.2f}".format(float(invoice[7] + vat)))
    print(invoice[4])
